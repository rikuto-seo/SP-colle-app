...
class GiveawayPhotoForm(FlaskForm):
    member = SelectField(
        ...
        validators=[
            DataRequired(message='メンバーを選択してください。')
        ],
    )
    costume = SelectField(
        '衣装',
        ...
        validate_choice=False,
        validators=[
            DataRequired(message='衣装を選択してください。')
        ],
    )
    photo_type = SelectField(
        ...
        validate_choice=False,
        validators=[
            DataRequired(message='種類を選択してください。')
        ],
    )
...
class UserPhotoImageUploadForm(FlaskForm):
    """ユーザーが所有する生写真の実物画像アップロード用フォーム。"""
